...
// Bahrain areas data for seeding
const bahrainAreasData = {
  governorates: [
    {
...
router.post('/seed-areas', async (req: Request, res: Response) => {
  try {
    ...
    console.log('🌱 Seeding Bahrain areas...');

    // Find or create Bahrain country
    let bahrain = await prisma.country.findFirst({ where: { code: 'BH' } });
    if (!bahrain) {
      bahrain = await prisma.country.create({
        data: { name: 'Bahrain', nameAr: 'البحرين', code: 'BH' },
      });
    }

    // Find or create state
    let bahrainState = await prisma.state.findFirst({ where: { countryId: bahrain.id } });
    if (!bahrainState) {
      bahrainState = await prisma.state.create({
        data: { countryId: bahrain.id, name: 'Kingdom of Bahrain', nameAr: 'مملكة البحرين', code: 'BH-00' },
      });
    }

    // Find or create district
    let bahrainDistrict = await prisma.district.findFirst({ where: { stateId: bahrainState.id } });
    if (!bahrainDistrict) {
      bahrainDistrict = await prisma.district.create({
        data: { stateId: bahrainState.id, name: 'Bahrain', nameAr: 'البحرين', code: 'BH-MAIN' },
      });
    }

    let totalAreas = 0;

    // Create governorates and areas
    for (const govData of bahrainAreasData.governorates) {
      ...
      for (const areaData of govData.areas) {
        ...
        } else if (!existingArea.isActive) {
          // Activate existing inactive area
          await prisma.area.update({
            where: { id: existingArea.id },
            data: { isActive: true },
          });
          console.log(`    Activated area: ${areaData.name}`);
        }
      }
    }
    ...
    res.json({
      success: true,
      message: 'Bahrain areas seeded successfully',
      data: {
        newAreasCreated: totalAreas,
        totalActiveAreas: areaCount,
        totalGovernorates: govCount,
      },
    });
  } catch (error: any) {
    ...
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to seed areas',
    });
  }
});
